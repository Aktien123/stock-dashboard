...
for ticker in tickers:
    ...
    chart_placeholders[ticker] = charts_container.empty()  # Platzhalter für Charts

# --------------------------
# Render Charts & KPIs einmal initial
# --------------------------
def render_all():
    ...
    for i, ticker in enumerate(tickers):
        ...
        with col:
            ...
            chart_placeholders[ticker].plotly_chart(fig, use_container_width=True)
...
# --------------------------
# Endlosschleife für KPI-Update + Balken
# --------------------------
...
while True:
    ...
    for sec in range(KPI_UPDATE_INTERVAL+1):
        ...
        time_placeholder.markdown(f"<div style='text-align:right; font-size:16px'>{datetime.now(tz_berlin).strftime('%H:%M')}</div>", unsafe_allow_html=True)
        time.sleep(1)
